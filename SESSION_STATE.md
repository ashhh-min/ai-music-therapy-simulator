...
- Last passing commands (2026-09-06, S19 implementation): `.venv/bin/python -m pytest` (281 passed via junit: tests=281 failures=0 errors=0 skipped=0; the console count line is swallowed by this environment - pre-existing quirk, junit is authoritative), `.venv/bin/ruff check src tests scripts` (clean), `scripts/smoke_test.py` (PASS), `git diff --check` clean, clean checkout with no key and no .env.local (smoke PASS + 281/281).
