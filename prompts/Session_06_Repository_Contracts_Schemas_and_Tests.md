...
- Confirm `S06` is the only active unit in `TASKS.md`.
- Record the pre-unit Git checkpoint.
- Run the currently passing baseline checks before editing.

## Session Objective
Strengthen the package contracts, validation, CI, and baseline tests without adding UI features.
...
- Save truthful command output or manual evidence under `evidence/S06/`.

## Acceptance Criteria
- Smoke and unit tests pass without API credentials.
- Schemas reject invalid/unlabelled records.
- CI does not depend on a live API.
...
## TASKS.md Update
Mark `S06` complete only after every acceptance item passes. Record changed files, actual test results, evidence path, unresolved issues, and prepare the next unit without implementing it.

## Stop Condition
Stop after `S06`. Report the diff, tests, evidence, issues, and next unit; do not begin the next prompt.
