...
- Confirm `S01` is the only active unit in `TASKS.md`.
- Record the pre-unit Git checkpoint.
- Run the currently passing baseline checks before editing.

## Session Objective
Freeze the research question, user-visible disclaimer, in-scope deliverables, exclusions, and success criteria.
...
- Save truthful command output or manual evidence under `evidence/S01/`.

## Acceptance Criteria
- Baseline loop remains recognizable.
- Every public claim distinguishes synthetic software behavior from clinical evidence.
- No implementation beyond bounded documentation changes.
...
## TASKS.md Update
Mark `S01` complete only after every acceptance item passes. Record changed files, actual test results, evidence path, unresolved issues, and prepare the next unit without implementing it.

## Stop Condition
Stop after `S01`. Report the diff, tests, evidence, issues, and next unit; do not begin the next prompt.
