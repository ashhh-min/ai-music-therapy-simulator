...
- Confirm `S15` is the only active unit in `TASKS.md`.
- Record the pre-unit Git checkpoint.
- Run the currently passing baseline checks before editing.

## Session Objective
Add six-dimension profiles, temporal stage views, descriptive rankings, uncertainty display, and exportable figures.
...
- Save truthful command output or manual evidence under `evidence/S15/`.

## Acceptance Criteria
- All requested baseline visualization types are represented or explicitly documented.
- Rankings are descriptive and show trial counts.
- No chart title implies treatment effectiveness.
...
## TASKS.md Update
Mark `S15` complete only after every acceptance item passes. Record changed files, actual test results, evidence path, unresolved issues, and prepare the next unit without implementing it.

## Stop Condition
Stop after `S15`. Report the diff, tests, evidence, issues, and next unit; do not begin the next prompt.
