...
- Confirm `S12` is the only active unit in `TASKS.md`.
- Record the pre-unit Git checkpoint.
- Run the currently passing baseline checks before editing.

## Session Objective
Complete AI/deterministic trial execution with start-middle-end sequence, uncertainty, safety flags, and failure handling.

## Files to Create or Modify
- `src/ai_music_therapy/ui/trial.py`
- `src/ai_music_therapy/models.py`
- `src/ai_music_therapy/ai_client.py`
- `tests/test_trial_workflow.py`
...
- Save truthful command output or manual evidence under `evidence/S12/`.

## Acceptance Criteria
- Both engines produce the same validated ReactionOutput schema.
- Invalid or refused AI output is not persisted.
- Safety flags and uncertainty are visible.
...
## TASKS.md Update
Mark `S12` complete only after every acceptance item passes. Record changed files, actual test results, evidence path, unresolved issues, and prepare the next unit without implementing it.

## Stop Condition
Stop after `S12`. Report the diff, tests, evidence, issues, and next unit; do not begin the next prompt.
