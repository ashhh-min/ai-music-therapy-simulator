...
- Confirm `S13` is the only active unit in `TASKS.md`.
- Record the pre-unit Git checkpoint.
- Run the currently passing baseline checks before editing.

## Session Objective
Create complete trial audit records, filtering, export, and evidence capture for accepted runs.

## Files to Create or Modify
- `src/ai_music_therapy/repository.py`
- `src/ai_music_therapy/ui/trial.py`
- `src/ai_music_therapy/export.py`
...
- Save truthful command output or manual evidence under `evidence/S13/`.

## Acceptance Criteria
- Every trial includes engine/model/prompt/seed/time/disclaimer.
- Exports retain synthetic labels and limitations.
- Duplicate IDs and incomplete records are rejected.
...
## TASKS.md Update
Mark `S13` complete only after every acceptance item passes. Record changed files, actual test results, evidence path, unresolved issues, and prepare the next unit without implementing it.

## Stop Condition
Stop after `S13`. Report the diff, tests, evidence, issues, and next unit; do not begin the next prompt.
