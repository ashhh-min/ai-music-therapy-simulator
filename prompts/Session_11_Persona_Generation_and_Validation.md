...
- Confirm `S11` is the only active unit in `TASKS.md`.
- Record the pre-unit Git checkpoint.
- Run the currently passing baseline checks before editing.

## Session Objective
Add an optional AI-assisted persona drafting workflow that must pass schema, diversity, and human-review gates before acceptance.

## Files to Create or Modify
- `src/ai_music_therapy/persona_service.py`
- `src/ai_music_therapy/ui/personas.py`
- `tests/test_persona_service.py`
- `docs/persona_review_checklist.md`
...
- Save truthful command output or manual evidence under `evidence/S11/`.

## Acceptance Criteria
- Generated profiles remain fictional and visibly synthetic.
- Human review can reject or revise stereotypes.
- No generated profile overwrites approved fixtures automatically.
...
## TASKS.md Update
Mark `S11` complete only after every acceptance item passes. Record changed files, actual test results, evidence path, unresolved issues, and prepare the next unit without implementing it.

## Stop Condition
Stop after `S11`. Report the diff, tests, evidence, issues, and next unit; do not begin the next prompt.
