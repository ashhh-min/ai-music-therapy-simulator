...
- Confirm `S05` is the only active unit in `TASKS.md`.
- Record the pre-unit Git checkpoint.
- Run the currently passing baseline checks before editing.

## Session Objective
Freeze the 5 × 5 × 3 synthetic trial matrix, hypotheses, variables, analysis plan, and exclusions before running trials.
...
- Save truthful command output or manual evidence under `evidence/S05/`.

## Acceptance Criteria
- Exactly 75 design cells are defined.
- Independent, dependent, controlled, and provenance variables are explicit.
- Analysis and exclusion rules are frozen before S16.
...
## TASKS.md Update
Mark `S05` complete only after every acceptance item passes. Record changed files, actual test results, evidence path, unresolved issues, and prepare the next unit without implementing it.

## Stop Condition
Stop after `S05`. Report the diff, tests, evidence, issues, and next unit; do not begin the next prompt.
