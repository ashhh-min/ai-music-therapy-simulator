...
- Confirm `S09` is the only active unit in `TASKS.md`.
- Record the pre-unit Git checkpoint.
- Run the currently passing baseline checks before editing.

## Session Objective
Implement a reproducible bounded rule engine for offline testing and demonstration.

## Files to Create or Modify
- `src/ai_music_therapy/deterministic_simulator.py`
- `tests/test_deterministic_simulator.py`
- `docs/deterministic_model.md`
...
- Save truthful command output or manual evidence under `evidence/S09/`.

## Acceptance Criteria
- Identical inputs yield identical seed and output.
- All scores remain in range and time stages are complete.
- Documentation states that rules are fictional and non-clinical.
...
## TASKS.md Update
Mark `S09` complete only after every acceptance item passes. Record changed files, actual test results, evidence path, unresolved issues, and prepare the next unit without implementing it.

## Stop Condition
Stop after `S09`. Report the diff, tests, evidence, issues, and next unit; do not begin the next prompt.
