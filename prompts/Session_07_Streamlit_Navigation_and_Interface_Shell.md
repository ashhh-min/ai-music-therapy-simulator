...
- Confirm `S07` is the only active unit in `TASKS.md`.
- Record the pre-unit Git checkpoint.
- Run the currently passing baseline checks before editing.

## Session Objective
Implement the multipage Streamlit shell, global disclaimer, empty states, and navigation only.

## Files to Create or Modify
- `app.py`
- `src/ai_music_therapy/ui/home.py`
- `src/ai_music_therapy/ui/personas.py`
- `src/ai_music_therapy/ui/trial.py`
- `src/ai_music_therapy/ui/dashboard.py`
- `src/ai_music_therapy/ui/methods.py`
...
- Save truthful command output or manual evidence under `evidence/S07/`.

## Acceptance Criteria
- All pages load with useful empty states.
- The disclaimer is visible on Home and trial/results surfaces.
- No future analytics or AI logic is implemented early.
...
## TASKS.md Update
Mark `S07` complete only after every acceptance item passes. Record changed files, actual test results, evidence path, unresolved issues, and prepare the next unit without implementing it.

## Stop Condition
Stop after `S07`. Report the diff, tests, evidence, issues, and next unit; do not begin the next prompt.
