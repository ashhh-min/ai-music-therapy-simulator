...
- Confirm `S08` is the only active unit in `TASKS.md`.
- Record the pre-unit Git checkpoint.
- Run the currently passing baseline checks before editing.

## Session Objective
Implement repository initialization, persona seed, trial persistence, and round-trip tests.

## Files to Create or Modify
- `src/ai_music_therapy/repository.py`
- `src/ai_music_therapy/seed_demo.py`
- `data/public/`
...
- Save truthful command output or manual evidence under `evidence/S08/`.

## Acceptance Criteria
- Five personas seed idempotently.
- Trial records preserve full provenance JSON.
- Local DB files remain ignored.
...
## TASKS.md Update
Mark `S08` complete only after every acceptance item passes. Record changed files, actual test results, evidence path, unresolved issues, and prepare the next unit without implementing it.

## Stop Condition
Stop after `S08`. Report the diff, tests, evidence, issues, and next unit; do not begin the next prompt.
