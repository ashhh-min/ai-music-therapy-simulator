...
## S17 - Analysis, Limitations, and Research Report (2026-08-21)

- What changed: three new documents only - `docs/analysis_notebook.md` (analysis record with the full 75-cell table, summary tables, H1-H4 probes, safety-flag report, engine-stability comparison), `docs/limitations.md` (persona validity, model dependence, prompt sensitivity, no human participants, coverage/reporting limits), `docs/ResearchReport.md` (final report; section 4A observed software behavior is separated from 4B model-generated content). No source code touched; the analysis read the immutable S16 bundles, and a new write-once AI comparison bundle (`ai-comparison-20260821`, 75 deterministic + 5 AI cells) was generated with `scripts/run_batch.py --ai-subset 5` to source the engine comparison.
- Files changed: `docs/ResearchReport.md` (new), `docs/analysis_notebook.md` (new), `docs/limitations.md` (new), `evidence/S17/check_outputs.txt` (new, local), control docs.
- Tests/checks run and actual results: pytest 227 passed (unchanged suite - documentation-only unit); ruff clean; smoke PASS; `git diff --check` clean; docs validation: 20 referenced paths exist, no broken links, claims-language scan clean; composite values in the notebook were programmatically verified against `analytics.composite_score`; reproducibility check 75/75 across the two bundles; live AI probes this session: 5-cell comparison bundle + 2 same-cell repeats (qwen3.8-max).
- Student explanation of one key decision: the report splits results into 4A (deterministic engine behavior, which is reproducible rule arithmetic) and 4B (AI model-generated content, which is not reproducible), because merging them would let a reader attribute rule-artifact patterns to model behavior or vice versa - the split keeps every number attributable to its engine.
- Errors or rejected suggestions: the first 5-cell AI comparison run hit the 2-minute command timeout (5 live calls are slower than one) - re-run with a longer timeout, no partial bundle was written (the runner only writes after full validation); an attempted `timeout` wrapper is absent on macOS zsh, removed; one typo fixed in limitations.md. The H4 caveat (per-stage mood/regulation not stored) is recorded as a schema limitation rather than silently substituting other fields.
- Evidence path: `evidence/S17/check_outputs.txt`.
- Mentor acceptance or required revision: unit complete pending mentor sign-off per workflow. Next unit: S18 (Deployment, Demo, Portfolio, and Release Audit) - pooled DB connections first, then Streamlit Community Cloud + Neon.

## S18 - Deployment, Demo, Portfolio, and Release Audit (2026-08-30)

- What changed: (hardening) new `src/ai_music_therapy/db.py` - ConnectionFactory, PooledConnectionManager over psycopg_pool (min 1 / max 8 / checkout 10 s / lifetime 1800 s), transaction() wrapper, run() with 3-attempt exponential-backoff retry on transient OperationalError, process-wide get_manager registry + atexit close; `src/ai_music_therapy/repository.py` refactored onto manager.run (public API unchanged); `tests/test_connection_pool.py` (7 new tests incl. 8-user simulated deployment); `pyproject.toml` + `requirements.txt` now `psycopg[binary,pool]`; requirements.txt also gains `-e .` for the Streamlit Cloud build of the src layout. (stability) `src/ai_music_therapy/ui/dashboard.py` migrated all 8 deprecated `use_container_width=True` to `width="stretch"`. (docs) `docs/DeploymentRunbook.md` rewritten; `docs/DemoScript.md` expanded (offline 5-7 min, fallbacks, claims discipline); `docs/PortfolioGuide.md` new; `docs/DeploymentGuide_StreamlitCloud_Neon.md` hardening prerequisite marked done + secrets/env decision recorded (Advanced settings env vars; no st.secrets code path); stale follow-up line in `docs/limitations.md` corrected; README operational sections (done earlier this unit).
- Files changed: `src/ai_music_therapy/db.py` (new), `src/ai_music_therapy/repository.py`, `tests/test_connection_pool.py` (new), `src/ai_music_therapy/ui/dashboard.py`, `pyproject.toml`, `requirements.txt`, `README.md`, `docs/DeploymentRunbook.md`, `docs/DemoScript.md`, `docs/PortfolioGuide.md` (new), `docs/DeploymentGuide_StreamlitCloud_Neon.md`, `docs/limitations.md` (one-line release-audit correction), `evidence/S18/check_outputs.txt` (new, local), control docs.
- Tests/checks run and actual results: pytest 234 passed (227 baseline + 7 pool tests); ruff clean; smoke PASS; `git diff --check` clean; pool facts verified (20 sequential ops opened <= 2 connections; mid-transaction raise rolls back; transient OperationalError retried and succeeds on attempt 3; UndefinedTable propagates immediately; 8 simulated users x 4 mixed ops through max_size=4, 32/32 succeeded, pool_size <= 4); AppTest no-key trial-page run saves a trial; batch `--dry-run` validates the frozen matrix; release scans clean (secrets: only local docker demo creds; private data: none; clinical claims: disclaimer/boundary contexts only; synthetic labels present); clean checkout (fresh tree + fresh venv + `pip install -e ".[dev]"` + smoke + pytest, no key, no .env.local) verified deterministic.
- Student explanation of one key decision: the process-wide get_manager registry. Streamlit reruns every page script on each interaction, so a Repository constructed per rerun must not open a new pool each time - before the fix, AppTest showed leaked pool worker threads. One cached pool per database URL, shared by all pages, reruns, and threads, with an atexit close hook, keeps connection use bounded, which is exactly what a small free-tier Neon database needs.
- Errors or rejected suggestions: psycopg_pool 3.3.x API mismatches fixed as encountered (no connection_func kwarg -> connection_class subclass routing through the factory to keep connection counting; no wait kwarg; get_stats() instead of stats()); per-rerun pool leak discovered via AppTest warnings and fixed with the registry; `use_container_width` is removed in current Streamlit so the dashboard migrated to `width="stretch"`; requirements.txt was missing the pool extra (cloud import would have crashed) - synced. The first live AI attempt earlier in the project was rejected mid-env-edit by the user and retried successfully (Bailian qwen3.8-max).
- Evidence path: `evidence/S18/check_outputs.txt`.
- Mentor acceptance or required revision: unit complete pending mentor sign-off per workflow. S18 is the FINAL unit of the S01-S18 plan: after acceptance the project is complete; the only open item is executing the cloud deployment checklist (DeploymentRunbook Parts C/D) with the student's Streamlit + Neon accounts.

## S19 - Ad-hoc Entrances: Staged Review and Approved Track/Persona Libraries (2026-09-06)

- What changed: (workflow) new `src/ai_music_therapy/track_service.py` - staged state machine `proposed -> under_review -> trial_confirmed -> approved | rejected` with a TRANSITIONS table, terminal states, `TrackProposal` (carries extraction + file name + sha256 only), `finalize_track` (approval-time entry builder), `reject`; (models/persistence) `models.py` gains `AudioExtraction` + `TrackEntry` (TRK- prefix, synthetic locked True, sha256 64-char); `repository.py` gains the `tracks` table (`CHECK (synthetic = 1)`) and write-once `save_track` / `get_track` / `list_tracks` (duplicate id refused, no update/delete path); (DSP) new `src/ai_music_therapy/audio_analysis.py` - local deterministic librosa extraction (tempo, RMS->volume bucket at researcher-defined -30/-14 dBFS thresholds, spectral centroid, duration, chroma-concentration-gated Krumhansl-Kessler major/minor), typed errors (corrupt/empty/too-long), `estimated_music_defaults` (never clamps out-of-range estimates into the ontology) and `merge_music` (validates + computes `overridden_fields` provenance); (UI) new pages `ui/upload_track.py` (upload wav/flac/ogg <= 20 MB with rights + no-retention notice -> extract -> editable review initialized from estimates -> in-memory confirmation trial -> approve commits TrackBase / reject clears everything) and `ui/propose_persona.py` (three tabs: JSON upload, key-gated AI draft reusing S11 `persona_service`, manual form; shared review with editable summary, lint hard flags disable approval, near-duplicate confirmation checkbox, overwrite refusal), shared `ui/confirm_trial.py` (T-PREVIEW in-memory trials, never saved), both registered in `app.py`; `ui/trial.py` gains a music-source radio so approved TrackBase tracks are selectable (exploratory cohort caption); `pyproject.toml`/`requirements.txt` gain librosa + soundfile; `docs/DataGovernance.md` gains the transient-uploaded-audio class and the ad-hoc upload handling / no-retention rule; README gains an "Ad-hoc entrances" section.
- Files changed: `src/ai_music_therapy/audio_analysis.py` (new), `src/ai_music_therapy/track_service.py` (new), `src/ai_music_therapy/ui/upload_track.py` (new), `src/ai_music_therapy/ui/propose_persona.py` (new), `src/ai_music_therapy/ui/confirm_trial.py` (new), `src/ai_music_therapy/models.py`, `src/ai_music_therapy/repository.py`, `src/ai_music_therapy/ui/trial.py`, `app.py`, `tests/test_track_library.py` (new, 15), `tests/test_audio_analysis.py` (new, 19), `tests/test_entrance_ui.py` (new, 13), `pyproject.toml`, `requirements.txt`, `README.md`, `docs/DataGovernance.md`, `docs/decisions.md` (D028), `evidence/S19/check_outputs.txt` (new, local), control docs. NOT touched: `config/trial_matrix.csv`, `config/music_ontology.json`, preregistration, analysis notebook, batch runner, immutable bundles, `persona_service.py` (reused as-is).
- Tests/checks run and actual results: full suite 281 passed / 0 failed / 0 errors / 0 skipped via junit XML (234 baseline + 47 new; this environment swallows pytest's final console count line - pre-existing, unrelated to S19 - so counts are junit-sourced); ruff clean; smoke PASS; `git diff --check` clean; clean checkout (fresh tree copy + fresh Python 3.13 venv + `pip install -e ".[dev]"`, no OPENAI_API_KEY, no .env.local) -> INSTALL_DONE, smoke PASS, 281/281; release scans clean (no secrets beyond the intentional local docker test-DB defaults, no clinical claims, synthetic labels present on both entrances, zero audio files committed - fixtures are generated in-memory signals); DB-gated tests executed against mt_simulator_test with zero skips.
- Student explanation of one key decision: the persistence boundary. The student asked for a review stage that can run a trial before anything joins the library, so the unit makes "approval" the ONLY database write in the whole flow: review state is ephemeral session state, confirmation trials carry T-PREVIEW ids and are never handed to `save_trial`, rejection discards everything, and an approved entry is write-once with no update path - so an accidental double-approve or a rejected draft can never corrupt TrackBase, and the raw audio itself is discarded before review even starts (only the profile + fingerprint can survive).
- Errors or rejected suggestions: (1) the first `advance()` signature took a state string but tests drove proposals - TypeError (unhashable TrackProposal) - fixed to mutate `proposal.state`, and stale internal callers were updated; (2) the major/minor correlation-MARGIN gate (>= 0.5) was rejected empirically: a clean C-major triad's best-vs-runner-up margin measured 0.123 while broadband noise measured 0.131 - not separable - replaced with a chroma-concentration gate (>= 2.0; triads ~4.2 vs noise ~1.07), with a None result when pitch structure is too flat to call; (3) AppTest could not click through `st.file_uploader` initially assumed undrivable - `set_value((name, bytes, mime))` actually works, so the real upload->extract path is tested end-to-end, while the review/approval stages are driven by seeding `session_state`; (4) `st.session_state.get(...)`/`at.session_state.get(...)` raised "get not found in session_state" under AppTest's state object - pages and tests switched to `in` + subscript access; (5) the first fresh-venv checkout attempt used system python3 (3.9.6 < required 3.11) and failed at install - re-run with the conda 3.13 interpreter; (6) one ruff B017 (blind-exception pytest.raises) fixed by asserting the concrete ValidationError.
- Evidence path: `evidence/S19/check_outputs.txt`.
- Mentor acceptance or required revision: ACCEPTED by the student 2026-09-06 after two same-day review revisions (mp3 + 120 MB cap; bounded AI-client timeout 120 s / no SDK transport retries + engine expectation hints). The student runs the cloud go-live steps (DeploymentRunbook Parts C/D, Streamlit Community Cloud + Neon) in an offline session. Next unit: S20 (parameter-to-Max/MSP patch generation, the reversed S19 flow) with its prompt prepared at prompts/Session_20_Parameter_to_MaxMSP_Patch_Generation.md; nothing begins until the student explicitly says to start S20.

- S19 revision during acceptance review (2026-09-06, commit follows c5fde84): student feedback - the 20 MB cap was too small for the common case (~100 MB CD-quality WAV) and compressed formats should be supported. Fix: `mp3` added to the accepted upload types (libsndfile 1.2.2, bundled with soundfile 0.14, decodes mp3 natively - verified by probe and a new round-trip test; zero new dependencies) and `MAX_UPLOAD_BYTES` raised to 120 MB (covers a 10-minute CD WAV = the 600 s duration cap, stays under Streamlit's 200 MB server default so no config.toml is needed locally or on Community Cloud); the upload stage now carries a format-size tip (mp3/flac/ogg for long tracks). Files: `audio_analysis.py` (error message), `ui/upload_track.py` (cap + type list + tip), `tests/test_audio_analysis.py` (+1 test), `README.md`, D028 correction note. Results: full suite 282 passed / 0 failed / 0 errors / 0 skipped (junit), ruff clean, smoke PASS, `git diff --check` clean, clean checkout re-verified (fresh tree + fresh venv, no key, no .env.local). Governance unchanged: uploaded mp3/wav bytes remain in-memory only; nothing about the persistence boundary moved.

- S19 second revision during acceptance review (2026-09-06): student report - the "openai" engine "processes endless" and shows no charts, unlike deterministic. Diagnosis by live probe: endpoint and key healthy (401 ping 0.13 s; minimal call 2.5 s) but a FULL ai_trial call against the reasoning model took 94.6 s, Streamlit renders only at script end, and neither `OpenAI(...)` construction had a timeout (SDK default 600 s + retries -> indefinite spinner on any stall). Fix: explicit `timeout=120 s`, `max_retries=0` in `ai_client.py` (constants reused by `persona_service.py`; the module's own single validation-retry kept, SDK transport retries dropped), plus a shared `ENGINE_HINT` caption on all three engine selectors setting the 1-2 minute expectation. Files: `ai_client.py`, `persona_service.py`, `ui/confirm_trial.py`, `ui/upload_track.py`, `ui/propose_persona.py`, `ui/trial.py`, `tests/test_ai_client.py` (+1), `tests/test_persona_service.py` (+1), `README.md`, D028 second-correction note, `evidence/S19/check_outputs.txt` section 10. Results: targeted 25/25, full suite 284 passed / 0 failed / 0 errors / 0 skipped (junit), ruff clean, smoke PASS, `git diff --check` clean. Nothing persisted differently; the frozen matrix and bundles were untouched.
