...
- [ ] S19 - Ad-hoc entrances: staged review and approved track/persona libraries (implemented 2026-09-06 with same-day review revision: mp3 + 120 MB cap; pending acceptance; 48 new tests, suite 282 green, evidence/S19/check_outputs.txt; prompt at prompts/Session_19_Adhoc_Entrances_Track_Persona_Libraries.md)
...
- Deterministic no-key path: full suite + smoke ran with no OPENAI_API_KEY; clean checkout (fresh tree copy, fresh Python 3.13 venv, `pip install -e ".[dev]"`, no key, no .env.local) passed smoke + 282/282 after the review revision.
