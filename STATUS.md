...
- Offline demo path: Available through deterministic mode (smoke + pytest pass; 282 tests incl. 7 connection-pool and 48 S19 entrance tests; demo script, portfolio guide, and deployment runbook complete (D027); research report + analysis notebook + limitations from immutable batch bundles, all descriptive and synthetic-labeled (D026); batch runner executes the frozen 75-cell matrix with no key (D025); clean checkout verified deterministic with no API key)
