...
- Offline demo path: Available through deterministic mode (smoke + pytest pass; 284 tests incl. 7 connection-pool, 48 S19 entrance tests, and 2 bounded-AI-client tests; demo script, portfolio guide, and deployment runbook complete (D027); research report + analysis notebook + limitations from immutable batch bundles, all descriptive and synthetic-labeled (D026); batch runner executes the frozen 75-cell matrix with no key (D025); clean checkout verified deterministic with no API key)
