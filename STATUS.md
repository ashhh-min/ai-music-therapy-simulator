...
- Active control step: S15 implemented 2026-08-20 (pending mentor acceptance); next unit S16
- Next implementation unit: S16 (Batch Experiment Runner and Trial Matrix) - not yet started
...
- Offline demo path: Available through deterministic mode (smoke + pytest pass; 213 tests; dashboard II adds radar profile, temporal stage means, rankings, uncertainty notes, figure export (D024); dashboard heatmap + same-music comparisons with counts/engines (D023); audit trail + labeled export live (D022); temporal sequence schema-enforced; trial UI shows uncertainty/safety/sequence (D021); persona drafting gated by schema/lint/human review (D020); ruff clean; AppTest shell loads; Postgres tests skip cleanly when the DB is down)
